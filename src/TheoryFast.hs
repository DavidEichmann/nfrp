...
iterateWhileHot = do
  ...
  case derIdMay of
    ...
    Just (SomeDerivationID vix derId) -> do
      ...
      let (mayNewFactsAndDers, (), deps) = runRWS (pokeDerivation der) (allFacts, allDers) ()
  ...
  pokeDerivation derivation = case derivation of
      Derivation dtrace ttspan contDerivation seenOcc -> case contDerivation of
            ...
            then Nothing
                ...
                mconcat
                  [ case fact of
                    ...
                    Left factTspan -> let
                      newCont = cont NoOcc
                      newDer  = Derivation dtrace factTspan newCont seenOcc
                                  `withDerTrace`
                    ...
                    Right (factT, b) -> let
                      newCont = cont (Occ b)
                      newDer  = Derivation dtrace (DS_Point factT) newCont True
                                `withDerTrace`
        ...
        PrevV eixB mayPrevToCont -> case ttspan of
          ...
          DS_SpanExc _ -> do
            (knownSpansAndValueMays, unknownSpans) <- spanLookupPrevV ttspan eixB
            return $ if null knownSpansAndValueMays
              then Nothing
              else Just $ (
                -- For knowns, split and try to progress the derivation.
                mconcat
...
withDerTrace (Derivation oldTrace ttspan cont seenOcc) msg =
  let dMsg = "Derivation "
                ...
                ++ ": " ++ msg
  in Derivation (oldTrace `appendDerTrace` dMsg) ttspan cont seenOcc
